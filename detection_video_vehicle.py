...
#PATH_TO_CKPT和PATH_TO_LABELS的位置一定要对，否则没法下一步，可以使用import os
#os.getcwd()来随时确认下当前目录的位置。
PATH_TO_CKPT = 'training\\frozen_inference_graph_doraemon.pb'

# List of the strings that is used to add correct label for each box.
PATH_TO_LABELS = os.path.join('training', 'object-detection-doraemon.pbtxt')
...
#    CV_FOURCC('F', 'L', 'V', '1') = FLV1 codec
    out = cv2.VideoWriter('doraemon_detection_storm.avi', cv2.VideoWriter_fourcc(
        'D', 'I', 'V', 'X'), 10, (1024, 576))
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            num_detections = detection_graph.get_tensor_by_name(
                'num_detections:0')
            cap = cv2.VideoCapture('z:/doraemon-space hero(00h10m20s-00h10m50s).mp4')
